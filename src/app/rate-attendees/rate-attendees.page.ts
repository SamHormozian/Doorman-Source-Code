...
@Component({
  selector: 'app-rate-attendees',
  templateUrl: './rate-attendees.page.html',
  styleUrls: ['./rate-attendees.page.scss'],
})
export class RateAttendeesPage implements OnInit {
  ...
  doSubmitReview(){
  	if(this.comment == ''){
	    ...
	    this.restApi.post("add_host_rating.php", params).subscribe((data: any) => {
	        ...
	        if (data.response.success == 1) {
	            this.loaderService.showToast(data.response.message);
	            this.route.navigate(['./rate-attendees-listing/'+this.eventId]);
	        }
	        else {
	            this.loaderService.showToast(data.response.message);
	        }
