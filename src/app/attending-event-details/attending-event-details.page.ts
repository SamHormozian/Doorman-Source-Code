...
@Component({
  selector: 'app-attending-event-details',
  templateUrl: './attending-event-details.page.html',
  styleUrls: ['./attending-event-details.page.scss'],
})
export class AttendingEventDetailsPage{
  isModalOpen = false;
  ...
  deleteApplicant(applicant_id){
    ...
    this.restApi.post("withdraw_from_event.php", params).subscribe((data: any) => {
        this.loaderService.dismissLoader();
        if (data.response.success == 1) {
            this.loaderService.showToast(data.response.message);
